...
app.get('/midc', (req,res)=> {
    const midcData = fs.readFileSync('./mappings/CrmgwDC.json', 
    ...
    for (const element of parsedMidcData) {
        if("pojo" == element["type"]) {
            ...
            const txnData = fs.readFileSync('./mappings/CrmgwTxn.json', 
            {encoding:'utf8', flag:'r'});
    ...
    fs.appendFile('./output/CrmgwMIDC.xls', content, (err) => {
        if (err) throw err;
